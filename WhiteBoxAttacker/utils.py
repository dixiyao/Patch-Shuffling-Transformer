...
def taylor_e(x, k):
    return torch.pow(x, k) / math.factorial(k)

def perm_e(x):
    # x=torch.exp(x)
    x = x / 64
    sum = torch.zeros((x.shape)).to(x.device)
    for i in range(1, 12):
        sum = sum + taylor_e(x, i)
    x = sum + 1
    x = torch.pow(x, 64)
    return x
...
def inverse(x):
    # x_inv=1/x
    x_inv = x / 1e4
    product = torch.ones(x_inv.shape).to(x.device)
    for i in range(10):
        product = product * (1 + torch.pow(1 - x_inv, math.pow(2, i)))
    x_inv = product / 1e4
    return x_inv

def approximate_sign(x):
    x=perm_e(-inverse(x*x))*x
    return x

def log_approx(x):
    sum=torch.zeros(x.shape).to(x.device)
    for i in range(1,10):
        sum=sum+torch.pow(x-1,i)/i*math.pow(-1,i-1)
    return sum

def sigmoid(x):
    return inverse(1+perm_e(-x))

class BCELosswithLogitis_Approx(torch.nn.Module):
    def __init__(self):
        super(BCELosswithLogitis_Approx, self).__init__()

    def forward(self, predict, targets):
        predict=sigmoid(predict)
        loss = -torch.mean(targets * torch.log(predict) + (1 - targets) * torch.log(1 - predict))
...
if __name__=="__main__":
    x=torch.rand((1,3,3))
    y=torch.zeros((1,3,3))
    bcecompare=torch.nn.BCEWithLogitsLoss()
    compare=bcecompare(x,y)
    bce=BCELosswithLogitis_Approx()
    x=bce(x,y)
    print(compare,x)