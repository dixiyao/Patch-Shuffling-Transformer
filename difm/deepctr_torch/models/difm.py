...
def PatchShuffle(x):
    row_perm = torch.rand((x.shape[0], x.shape[1],x.shape[2])).argsort(1).to(x.device)
    perm_back = row_perm.argsort(1)
    x = x.gather(1, perm_back)
    return x
...
class DIFM(BaseModel):
    ...
    def forward(self, X):
        ...
        # shuffle part
        att_input=BatchPatchPartialShuffle(att_input,1.,0)
        #####################################################
        # noise = att_input.data.new(att_input.size()).normal_(0, 0.001)
        # att_input = att_input + noise
        # att_input = att_input.data.new(att_input.size()).normal_(0, 1)
        # print(torch.norm(att_input,p='fro'))
        # print(torch.mean(torch.abs(att_input)))
