...
from .basemodelAttack import BaseModel
...
class DIFMAttack(BaseModel):
    ...
    def __init__(self,
        ...
        super(DIFMAttack, self).__init__(linear_feature_columns, dnn_feature_columns, l2_reg_linear=l2_reg_linear,
        ...
        self.vector_wise_net_reverse = InteractingLayer(self.embedding_size, att_head_num,
                                                att_res, scaling=True, device=device)
        ...
        self.transform_matrix_P_vec_reverse = nn.Linear(
            self.sparse_feat_num, self.sparse_feat_num * self.embedding_size, bias=False).to(device)
        self.transform_matrix_P_bit = nn.Linear(
            dnn_hidden_units[-1], self.sparse_feat_num, bias=False).to(device)

        self.final=torch.nn.Sequential(Mlp(26,out_features=1024),
    ...
    def forward(self, X):
        with torch.no_grad():
            ...
            #####################################################3
            # idx = torch.randperm(att_input.shape[1])
            # att_input = att_input[:, idx].view(att_input.size())
            # # shuffle code
            # for bs in range(att_input.shape[0]):
            #     # random permutation
            #     att_input[bs] = att_input[bs][torch.randperm(att_input.shape[1]), :]
            #####################################################
            att_out = self.vector_wise_net(att_input)
            att_out = att_out.reshape(att_out.shape[0], -1)
            m_vec = self.transform_matrix_P_vec(att_out)

        m_vec = self.transform_matrix_P_vec_reverse(m_vec)
        m_vec = m_vec.reshape(m_vec.shape[0], 26,4)
        m_vec = self.vector_wise_net_reverse(m_vec)
        ...
        # refined_fm_input=rearrange(refined_fm_input, 'b n d -> b (n d)')
        # logit=torch.cat([logit,refined_fm_input],dim=1)
        # y_pred = self.final(m_vec)

        return m_vec,att_input
