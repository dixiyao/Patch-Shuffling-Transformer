...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    for _, (x_train, y_train) in t:
                        ...
                        y_pred,ylabel = model(x)

                        optim.zero_grad()
                        loss = loss_func(y_pred, ylabel)#, reduction='sum')
                        ...
                        if verbose > 0:
                            for name, metric_fun in self.metrics.items():
                                if name not in train_result:
                                    train_result[name] = []
                                train_result[name].append(loss.item())#metric_fun(
            ...
            if do_validation:
                ...
                epoch_logs["val_mse"] = eval_result
            # verbose
            if verbose > 0:
    ...
    def evaluate(self, x, y, batch_size=256):
        ...
        """
        eval_result=self.predict(x, batch_size)
    ...
    def predict(self, x, batch_size=256):
        ...
        with torch.no_grad():
            for _, x_test in enumerate(test_loader):
                ...
                y_pred,ylabel = model(x)
                loss+=torch.nn.MSELoss()(y_pred,ylabel)*x.shape[0]
                count+=x.shape[0]
