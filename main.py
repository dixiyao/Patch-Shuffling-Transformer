...
parser.add_argument('--k', type=float, default=1.,
                    help='BatchPatchShuffle Parameter k not using class attention')
...
net_kernel = t2t_vit_24(num_classes=args.num_classes)

print('transfer learning, load t2t-vit pretrained model')
load_for_transfer_learning(net_kernel, args.transfer_model, use_ema=True, strict=False, num_classes=args.num_classes)
net = model.t2t_pretrain(net_kernel, k1=args.k)
...
if args.dataset == 'cifar10' or args.dataset == 'cifar100' or args.dataset == 'imdb':
    criterion = nn.CrossEntropyLoss()
else:
    criterion = nn.BCEWithLogitsLoss()

# set optimizer
parameters = [{'params': net.model.blocks[1:].parameters(), 'lr': args.transfer_ratio * args.lr},
...
def train(epoch):
    ...
    print('train epoch %3d,acc: %.3f'%(epoch,acc))
    global log_loss
